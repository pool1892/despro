...
newtype Field = Field (Array.Array Pos (Maybe Player))
...
initialField :: Field
initialField = Field (Array.listArray ((A,X),(C,Z)) (repeat Nothing))
-- listArray nimmt ein Tupel (anfang, ende) und eine Liste und erzeugt ein Array

infixl 9 !
(!) ::  Field -> Pos -> Maybe Player
(!) (Field f) pos = (Array.!) f pos
-- Arrays haben schon eine Lookup-Funktion

possibleMoves :: Field -> [Pos]
possibleMoves f = [ (r,c) | r <- [A .. C], c <- [X .. Z ], isNothing (f ! (r,c))]

makeMove :: Player -> Pos -> Field -> Field
makeMove player pos (Field f) = Field ((Array.//) f [(pos, Just player)])
-- // ist das Update von Arrays
...
und
  any :: (a -> Bool) -> [a] -> Bool
prüfen ob eine Bedingung für alle Elemente einer Liste erfüllt ist.
hasline prüft also ob alle Felder von line dem Spieler player gehören.
Der Guard any (hasLine XPlayer) winningLines prüft, ob es eine Linie 
gibt, die komplett von einem Spieler besetzt ist.
-}
